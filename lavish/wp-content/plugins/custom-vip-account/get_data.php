    ...
    <section class="vip-form-div">
      <div class="vip-form-box-left clearfix">
       ...
       <li class="vip-form-box-field">
        ...
        <div class="vip-fields"> <input placeholder="" name="cityds" value="" type="text" required> </div>
       </li>
       <li class="vip-form-box-field">
        <div class="vip-label">
       ...
       <li class="vip-form-box-field">
        ...
        <div class="vip-fields"> <input placeholder="" name="username" id="username" value="" type="text" required> </div>
       </li>
	   <li class="vip-form-box-field">
        <div class="vip-label">
       ...
       <li class="vip-form-box-field">
        ...
        <div class="vip-fields"> <input placeholder="" type="password" name="password" id="password" value="" required> </div>
       </li>
       <li class="vip-form-box-field">
        <div class="vip-label">
         <label> Confirm Password <span class="required">*</span> </label> </div>
        <div class="vip-fields"> <input placeholder="" type="password" name="conpassword" id="conpassword" value="" type="password" required> </div>
       </li>
       ...
       <li class="vip-form-box-field">
        ...
        <div class="vip-fields"> 
           <select name="total_plan" id="total_plans">
             <option name="Lavish $150" value="Lavish $150" <?php if($pac_name == "lavishpln") echo "selected";?>>Lavish $150</option>
             <option name="Silver $100k" value="Silver $100k" <?php if($pac_name == "sliverpln") echo "selected";?>>Silver $100k</option>
             <option name="Gold $250k" value="Gold $250k" <?php if($pac_name == "goldpln") echo "selected";?>>Gold $250k</option>
             <option name="Platinum $500k" value="Platinum $500k" <?php if($pac_name == "platinumpln") echo "selected";?>>Platinum $500k</option>
...
<li>
						   <label><input type="checkbox" value="<?php print_r($recent_posts['post_title'] );?>" /><?php print_r($recent_posts['post_title'] );?> </label>
						</li>
...
function complete_registration($username,$first_name,$last_name,$age,$nationality,$address,$cityds,$zipcode,$username,$password,
$conpassword,$email,$conemail,$phone,$mob_num,$time_call,$explain,$hot_spot,$toys,$gateway,$mem_pln,$total_plans) {
	global $wpdb;
    global $reg_errors, $username,$first_name,$last_name,$age,$nationality,$address,$cityds,$zipcode,$username,$password,
$conpassword,$email,$conemail,$phone,$mob_num,$time_call,$explain,$hot_spot,$toys,$gateway,$mem_pln,$total_plans;
    if ( 1 > count( $reg_errors->get_error_messages() ) ) {
        ...
        'last_name'     =>   $last_name,
         ...
         //~ add_user_meta( $user, 'address', $address_events);
		add_user_meta( $user, 'phone', $phone );
		add_user_meta( $user, 'mobphone', $mob_num );
	...
	add_post_meta($pid, 'vip_account_deatils_first_name',$first_name, true);
	add_post_meta($pid, 'vip_account_deatils_last_name',$last_name, true);
	add_post_meta($pid, 'vip_account_deatils_age',$age, true);
	add_post_meta($pid, 'vip_account_deatils_nationality',$nationality,true );
	add_post_meta($pid, 'vip_account_deatils_address',$address,true );
	add_post_meta($pid, 'vip_account_deatils_city',$cityds,true );
	add_post_meta($pid, 'vip_account_deatils_zip_code',$zipcode,true  );
	add_post_meta($pid, 'vip_account_deatils_user_name',$username,true  );
	add_post_meta($pid, 'vip_account_deatils_e_mail',$email,true );
	add_post_meta($pid, 'vip_account_deatils_password',$password,true );
	add_post_meta($pid, 'vip_account_deatils_phone_number',$phone,true );
	add_post_meta($pid, 'vip_account_deatils_mobile_phone_number',$mob_num,true );
	add_post_meta($pid, 'vip_account_deatils_best_times_to_call',$time_call,true );
	add_post_meta($pid, 'vip_account_deatils_if_wish_not_be_contacted',$explain,true );
	add_post_meta($pid, 'vip_account_deatils_membership',$total_plans,true );
	add_post_meta($pid, 'vip_account_deatils_what_are_your_favorite_luxury_hot_spots_',$hot_spot,true );
	add_post_meta($pid, 'vip_account_deatils_what_are_your_favorite_luxyry_toys_',$toys,true);
	add_post_meta($pid, 'vip_account_deatils_what_would_be_your_dream_getaway_',$gateway,true );
       ...
       $sql = "INSERT INTO `unitetoh_lav`.`wp_vip_accounts` (`vip_account_id`, `vip_account_name`, `vip_account_package_name`, `vip_account_post_limit`, `vip_account_access`, `vip_user_id`, `is_payment`, `vip_first_name`, `vip_last_name`, `vip_nationality`, `vip_address`, `vip_cityds`, `vip_zipcode`, `vip_email`, `vip_phone`, `vip_mob_num`, `vip_time_call`, `vip_explain`, `vip_hot_spot`, `vip_toys`, `vip_gateway`, `vip_monthy_package`, `vip_total_package`) VALUES 
       ('', '$username', 'Sliver', '130', 'all', '$uid', '0', '$first_name', '$last_name', '$nationality', '$address', '$cityds', '$zipcode', '$email', '$phone', '$mob_num', '$time_call', '$explain', '$hot_spot', '$toys', '$gateway', '$mem_pln', '$total_plans');";
        ...
        //$user = wp_insert_user( $userdata );
        dbDelta( $sql );
        $url = esc_url(home_url())."/payment-vip?uid=".$uid;
...
function custom_registration_function() {
    if(isset($_POST['submit']) && $_POST['username'] != "") {
        ...
        global $username,$first_name,$last_name,$age,$nationality,$address,$cityds,$zipcode,$username,$password,
$conpassword,$email,$conemail,$phone,$mob_num,$time_call,$explain,$hot_spot,$toys,$gateway,$mem_pln,$total_plans;
...
foreach($recent_posts as $recent_posts)
{
		...
		$first_name = get_post_meta( $recent_posts['ID'], 'vip_account_deatils_first_name', true ); 
		$last_name =    get_post_meta( $recent_posts['ID'], 'vip_account_deatils_last_name', true ); 
		$age =    get_post_meta( $recent_posts['ID'], 'vip_account_deatils_age', true ); 
		$address =     get_post_meta( $recent_posts['ID'], 'vip_account_deatils_address', true );  
		$city =     get_post_meta( $recent_posts['ID'], 'vip_account_deatils_city', true );     
		$zipcode =     get_post_meta( $recent_posts['ID'], 'vip_account_deatils_zip_code', true );  
		$email =     get_post_meta( $recent_posts['ID'], 'vip_account_deatils_e_mail', true );    
		$phone =    get_post_meta( $recent_posts['ID'], 'vip_account_deatils_phone_number', true );     
		$mobile_number =get_post_meta( $recent_posts['ID'], 'vip_account_deatils_mobile_phone_number', true ); 
		
		$package_name = get_post_meta( $recent_posts['ID'], 'vip_account_deatils_membership', true ); 
		$price_org = intval(preg_replace('/[^0-9]+/', '', $package_name), 10);
		//print_r($int);
}
...
//~ $sql = "Select * from `unitetoh_lav`.`wp_vip_accounts` where `vip_user_id`=$uid;";
...
//~ }
   if($pid !="")
   {
	  ...
	  $duration = get_post_meta( $pid, 'booking_forms_duration', true );
	  $gender = get_post_meta( $pid, 'booking_forms_gender', true );
	  if(substr_count($duration,"2 Hours")>0)
	  {
		$price_org ="900";
	  }
	  if(substr_count($duration,"3 Hours")>0)
	  {
		$price_org ="1300";
	  }
	  if(substr_count($duration,"4 Hours")>0)
	  {
		$price_org ="1700";
	  }
	  if(substr_count($duration,"6 Hours")>0)
	  {
		$price_org ="2500";
	  }
	  if(substr_count($duration,"1 Night")>0)
	  {
		$price_org ="3600";
	  }
	  if(substr_count($duration,"1 Day")>0)
	  {
		$price_org ="4800";
	  }
	  
	  if($gender=="We are couple")
	  {
		  $price_org = $price_org*1.2;
	  }
   }
...
elseif (!empty($_GET['token-id'])) 
{
 ...
 $pid = $_REQUEST['pid'];
    ...
    if ((string)$gwResponse->result == 1 ) 
    {
		...
		update_post_meta( $pid, 'booking_forms_payment_status', '1' );
		$url1 = esc_url(home_url())."/dashboard?pid=".$pid;
	 ...
	 //header('Location: '.$url);   
		...
		else
		{
			...
			foreach($recent_posts as $recent_posts)
			{
					$post_id = $recent_posts['ID'];
			}
			
			
			
			update_post_meta( $post_id, 'vip_account_deatils_payment_status', '1' );
			update_post_meta( $post_id, 'vip_account_deatils_cc', $cc_number );
				
			$url2 = esc_url(home_url())."/dashboard?uid=".$uid;
