...
function clean_commerce_widgets_init() {
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Woocommerce Sidebar', 'clean-commerce' ),
...
function sb_loop_columns() {
return 4; // 5 products per row
}
add_filter('loop_shop_columns', 'sb_loop_columns', 999);

// Display 10 products per page. 
add_filter( 'loop_shop_per_page', create_function( '$cols', 'return 10;' ), 20 );

//disable variable product price display from single and archive pages
add_filter('woocommerce_variable_price_html','sb_hide_variable_price',10);
add_filter('woocommerce_grouped_price_html','sb_hide_variable_price',10);
add_filter('woocommerce_variable_sale_price_html','sb_hide_variable_price',10);
function sb_hide_variable_price($price){
    return false;
}