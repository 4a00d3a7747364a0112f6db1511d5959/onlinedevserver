...
<h2> My Diapers Gift Subscription 
	<?php
		...
		<?php foreach ( $customer_memberships as $customer_membership ) : ?>
			...
			<tr class="membership">
					...
					<?php if ( 'membership-plan' === $column_id ) : ?>
						...
						<td class="membership-plan" data-title="<?php echo esc_attr( $column_name ); ?>">
							...
							<?php if ( wc_memberships_is_user_active_member( get_current_user_id(), $customer_membership->get_plan() ) && ! empty ( $members_area ) && is_array( $members_area ) ) : ?>
								<?php /*?><a href="<?php echo esc_url( wc_memberships_get_members_area_url( $customer_membership->get_plan_id(), $members_area[0] ) ); ?>"><?php echo esc_html( $customer_membership->get_plan()->get_name() ); ?></a><?php */?>
					...
					<?php elseif ( 'membership-actions' === $column_id ) :
						...
						<td class="membership-actions order-actions" data-title="<?php echo esc_attr( $column_name ); ?>">
							<?php
								...
								");
							$postds_id=$customer_membership->plan_id;
							$product_ids = get_post_meta( $postds_id, '_product_ids', true );
							
						 if($customer_membership->get_status()=='expired'){
				...
				//$interval =$end_date -  $start_date;
						...
						echo $end_date->format('%R%d days, %H hours');*/
						
	// echo human_time_diff( $start_date, current_time('timestamp') ) . ' ago'; 
	 
	 /* echo human_time_diff( $start_date, $end_date) . ' ago'; 
 echo date_i18n( wc_date_format(), $end_date ); */

							?>
                           <a href="?add-to-cart=<?php echo $product_ids[0];?>&reactive=1" class="button view"> Gift Re-Subscription</a>
                           <?php
						 }
						 ?>
						</td>
