...
class EMAIL_TO_DB {
 ...
 var $file_path = '/home/unitetoh/public_html/onlinedevserver.biz/dev/demo/email/dsstoreattachment/'; #Where to write file attachments to /var/www/files/email/
 ...
 var $this_file_name = 'emailaction.php'; #If mode "html" 
...
function getcontentmail(){
	$websiteuel='http://onlinedevserver.biz/dev/demo/email/';
	$totall=18;	 
	$emailto = $_REQUEST['EmailTo'];
	
$usexecute = mysql_query("select max(ID) from emailtodb_email");
  $usrow = mysql_fetch_array($usexecute);
  $usID = $usrow[0];
   $usexecuted = mysql_query("SELECT * FROM `emailtodb_email` WHERE `ID` =".$usID );
  $usrowd = mysql_fetch_array($usexecuted);
 /* if($usrowd['EmailTo']==''){
      ...
      // echo "E-mail extract ddddddddd"; 
	 

	  
	  if(empty($emailto)){
		  $emailto = $usrowd['EmailTo'];
		  }
	  $exeemailits = mysql_query("SELECT * FROM `emailtodb_email` GROUP BY `EmailTo` ");
	  echo '<select>';
	   while($uee = mysql_fetch_array($exeemailits)){
		   echo "<option value='".$uee['EmailTo']."'>".$uee['EmailTo']."</option>";
		   }
		 echo '<select>';
	  
	    $executeds = mysql_query("SELECT * FROM `emailtodb_email` WHERE `EmailTo` = '".$emailto."'");
		$noleave = mysql_num_rows($executeds);
		$namedsd=explode('@',$emailto);
		$name=$namedsd[0];
		$lv =  leavecounttb($emailto);
		//print_r($lv);
		$emailds.='<table border="1">';
	 $emailds.='<tr>'; 
    $emailds.='<td colspan="4">Name :'.$name.'<br />
Email:'.$emailto.'<br />
Total Leave Application Count:'.$lv['total'].'<br />
Total Reject Leave Count:'.$lv['re'].'<br />
Total Approve Leave Count:'.$lv['accpt'].'<br />
Total Remaining Leave Count:'.($totall-$lv['accpt']).'<br />
<br />
Email Content:<br />
'.$row['Message'].'<br />
<br />
</td>';
/*'<a href="emailaction.php?st='.$usID.'&sta=a"><img src="approve_user.png" title="approve"> </a> || <a href="emailaction.php?st='.$usID.'&sta=d"><img src="user_deny.png" title="deny"></a>';*/
   	$emailds.='</tr>'; 
   ...
   $emailds.='<td>&nbsp;</td>';
   	$emailds.='</tr>'; 
	$emailds.='<tr bgcolor="#330000" style="color: #fff;
font-weight: bold;
text-align: center;">'; 
    $emailds.='<td>Date</td>';
   $emailds.='<td>Subject</td>';
    $emailds.='<td></td>';
   $emailds.='<td>&nbsp;</td>';
   	$emailds.='</tr>'; 
   while($row = mysql_fetch_array($executeds)){
	   $emailds.='<tr>'; 
    $ID = $row['ID'];
	$date=date_create($row['DateE']);
    $emailds.='<td>'. date_format($date,"g:ia \o\n l jS F Y").'</td>';
   $emailds.='<td>'.$row['Subject'].'</td>';
    $emailds.='<td width="400">'.limit_words($row['Message'], 10).'</td>';
   $emailds.='<td>'.elst($ID).'<a href="'.$websiteuel.'emailaction.php?st='.$ID.'&sta=a"><img src="'.$websiteuel.'approve_user.png" title="approve" style="width: 45px;"> </a> || <a href="'.$websiteuel.'emailaction.php?st='.$ID.'&sta=d"><img src="'.$websiteuel.'user_deny.png" title="deny" style="width: 45px;"></a></td>';
   	$emailds.='</tr>'; 
   }
   	$emailds.='</table>';
   echo  $emailds;
  ...
  $to = 'dilip@uniterrene.com';

$subject = 'Leave Infromation of '.$name;

$from = 'pms@uniterrene.com';
...
function getleavevalue($id,$status){
	$exchk=mysql_query("SELECT * FROM `emailtodb_emailstatus` WHERE `EmilId` =".$id);
$nocount = mysql_num_rows($exchk);
if($nocount>0){
	$ids=mysql_fetch_array($exchk);
	$action='UPDATE';
	}else{
		$action='INSERT';
		}
	...
	if($action=='INSERT'){
	mysql_query("INSERT INTO `unitetoh_emaildata`.`emailtodb_emailstatus` (
`IdSt` ,
`EmilId` ,
`EmailUser` ,
`StatusE` ,
...
if($action=='UPDATE'){
	...
	mysql_query("UPDATE `unitetoh_emaildata`.`emailtodb_emailstatus` SET  `EmailUser` = '', `StatusE` = '$status', `StatusTime` = NOW() WHERE `emailtodb_emailstatus`.`IdSt` =".$ids['IdSt']);
	}
	
	}
function elst($id){
	$exchkddd=mysql_query("SELECT * FROM `emailtodb_emailstatus` WHERE `EmilId` =".$id);
	$idst=mysql_fetch_array($exchkddd);
	return $idst['StatusE'];
	}
function leavecounttb($email){
$totall=array();
	$rej=array();
	$accs=array();
	$xxx=array();
	
	$exl=mysql_query("SELECT *  FROM emailtodb_email  WHERE EmailFrom = '$email'");
$exlre=mysql_query("SELECT * FROM emailtodb_email e, emailtodb_emailstatus es WHERE e.ID = es.EmilId AND e.EmailFrom = '$email' AND es.StatusE = 'NO'");
$exlaccpt=mysql_query("SELECT * FROM emailtodb_email e, emailtodb_emailstatus es WHERE e.ID = es.EmilId AND e.EmailFrom = '$email' AND es.StatusE = 'YES'");

	$norows = mysql_num_rows($exl);
	$rej = mysql_num_rows($exlre);
	$accpt=mysql_num_rows($exlaccpt);
	 $xxx['total']=$norows;
	 $xxx['re']=$rej;
	 $xxx['accpt']=$accpt;
	 return $xxx;
