...
DROP TABLE IF EXISTS {$this->getTable('shipping_premiumzone')};
CREATE TABLE {$this->getTable('shipping_premiumzone')} (
  zone_id int(10) unsigned NOT NULL auto_increment,
  website_id int(11) NOT NULL default '0',
  dest_zone varchar(40) NOT NULL default '',
  ...
  PRIMARY KEY(`zone_id`),
  UNIQUE KEY `dest_zone` (`website_id`,`dest_zone`,`dest_country_id`,`dest_region_id`,`dest_city`,`dest_zip`,`dest_zip_to`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;


DROP TABLE IF EXISTS {$this->getTable('shipping_premiumzonematrix')};
CREATE TABLE {$this->getTable('shipping_premiumzonematrix')} (
  ...
  PRIMARY KEY(`pk`),
  UNIQUE KEY `dest_zonematrix` (`website_id`,`dest_zone`,`condition_name`,`weight_from_value`,`weight_to_value`,`price_from_value`,`price_to_value`,`item_from_value`,`item_to_value`,`delivery_type`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
