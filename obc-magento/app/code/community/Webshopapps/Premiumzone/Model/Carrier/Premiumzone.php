...
class Webshopapps_Premiumzone_Model_Carrier_Premiumzone
    extends Webshopapps_Wsacommon_Model_Shipping_Carrier_Baseabstract
    implements Mage_Shipping_Model_Carrier_Interface
{

    protected $_code = 'premiumzone';
...
//asdasd
    ...
    public function setRequest(Mage_Shipping_Model_Rate_Request $request)
    {
    	...
    	$request->setPZConditionName($this->getConfigData('condition_name') ? $this->getConfigData('condition_name') : $this->_default_condition_name);
    ...
    {    	
    	...
    	{
        	$result = Mage::getModel('premiumzone_shipping/rate_result');
        } else 
        {
        ...
        if (($version == 1.6 || $version == 1.7 || $version == 1.8) ) {
        	...
        	if ($request->getFreeShipping() === true || $request->getPackageQty() == $this->getFreeBoxes() ) {
				...
				$method->setCarrier('premiumzone');
				$method->setCarrierTitle($this->getConfigData('title'));
				$method->setMethod(strtolower('premiumzone_'.$this->getConfigData('free_method_text')));
    ...
    {
    	return Mage::getResourceModel('premiumzone_shipping/carrier_premiumzone')->getNewRate($request);
    }   
    ...
    private function setTotalPrice($request, $discounted, $taxed)
    {
        ...
        if (is_array($items))
        {
            ...
            if (Mage::helper('wsalogger')->isDebug('Webshopapps_Premiumzone')) {
                Mage::helper('wsalogger/log')->postDebug('premiumzone','Original Package Weight',$request->getPackageWeight());
                Mage::helper('wsalogger/log')->postDebug('premiumzone','Original Package Value',$request->getPackageValue());
                Mage::helper('wsalogger/log')->postDebug('premiumzone','Original Package Qty',$request->getPackageQty());
                Mage::helper('wsalogger/log')->postDebug('premiumzone','New Package Weight',$totalWeight);
                Mage::helper('wsalogger/log')->postDebug('premiumzone','New Package Value',$totalPrice);
                Mage::helper('wsalogger/log')->postDebug('premiumzone','New Package Qty',$totalQty);
            }
