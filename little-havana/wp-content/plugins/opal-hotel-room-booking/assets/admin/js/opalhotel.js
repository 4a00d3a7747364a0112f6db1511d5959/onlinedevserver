			...
			booking: {
				...
				submit_modal: function( e, target, data ) {
					...
					if ( typeof data.action !== 'undefined'
						&& ( target === 'opalhotel-confirm' || target === 'opalhotel-template-order-item' )
					) {
						var _body = $( 'body' );
						if ( data.action === 'opalhotel_remove_order_item'
							|| data.action === 'opalhotel_order_remove_coupon'
						) {
							$.ajax({
								url: ajaxurl,
								type: 'POST',
