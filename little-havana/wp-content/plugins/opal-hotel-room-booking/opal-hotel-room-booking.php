...
if ( ! class_exists( 'OpalHotel' ) ) {
	...
	final class OpalHotel {
		...
		public function __construct() {
			...
			/* register active hook process install options, pages default for our system */
	        ...
	        add_action( 'wpmu_new_blog', array( 'OpalHotel_Install','create_new_blog' ), 10, 6 );
	        // multisite delete table in multisite
	        add_filter( 'wpmu_drop_tables', array( 'OpalHotel_Install', 'delete_tables' ) );
		...
		public function defined(){
			...
			if ( ! defined( 'OPALHOTEL_PATH' ) ) {
				define( 'OPALHOTEL_PATH', untrailingslashit( plugin_dir_path( __FILE__ ) ) );
			}
			/* define plugin url */
			if ( ! defined( 'OPALHOTEL_URI' ) ) {
				define( 'OPALHOTEL_URI', plugin_dir_url( __FILE__ ) );
			}
			/* define languages directory path */
			if ( ! defined( 'OPALHOTEL_LANG_PATH' ) ) {
				define( 'OPALHOTEL_LANG_PATH', OPALHOTEL_PATH . '/languages' );
			}
			/* define includes directory path */
			if ( ! defined( 'OPALHOTEL_INC_PATH' ) ) {
				define( 'OPALHOTEL_INC_PATH', OPALHOTEL_PATH . '/includes' );
			}
			/* define plugin current version */
			if ( ! defined( 'OPALHOTEL_VERSION' ) ) {
				define( 'OPALHOTEL_VERSION', '1.1.6.4' );
			}
			/* option group */
			if ( ! defined( 'OPALHOTEL_SETTING_GROUP_NAME' ) ) {
				define( 'OPALHOTEL_SETTING_GROUP_NAME', 'opalhotel_' );
			}
		}
		/** 
		 * include files 
		...
		/** 
		 * Hook actions while installing
		 */
		public function install() {
			return OpalHotel_Install::install();
		}
