...
class OpalHotel_Checkout {
	...
	public function create_order( $posted = null ) {
		...
		try {
			...
			foreach ( $rooms as $cart_room_id => $room ) {
				...
				$arrival = isset( $room['arrival'] ) ? absint( $room['arrival'] ) : 0;
				$departure = isset( $room['departure'] ) ? absint( $room['departure'] ) : 0;
			...
			add_post_meta( $order_id, '_arrival', $arrival );
			add_post_meta( $order_id, '_departure', $departure );
	...
	public function process_checkout() {
		...
		try {
			...
			/* check room available again. make sure rooms is available */
			do_action( 'opalhotel_checkout_room_available' );
				...
				if ( OpalHotel()->cart->need_payment() ) {
					...
					if ( isset( $result['status'] ) && $result['status'] === true ) {
						if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
							...
							wp_redirect( $result['redirect'] ); exit;
						}
					}
				} else {
					if ( empty( $order_id ) ) {
