			...
			<div class="opalhotel_backbone_modal_content">
				<form>
					...
					<footer class="center">
						...
						<button type="submit" class="opalhotel-button opalhotel-button-submit button button-primary"><?php _e( 'Yes', 'opal-hotel-room-booking' ); ?></button>
						<button type="reset" class="opalhotel-button-cancel opalhotel_button_close button"><?php _e( 'No', 'opal-hotel-room-booking' ) ?></button>
				...
				<form>
					<header>
						<h2>
							<# if ( data.message ) { #>
								<!-- {{{ data.message }}} -->
							<# } else { #>
								<?php _e( 'Add new item', 'opal-hotel-room-booking' ); ?>
							<# } #>
							<select style="width: 200px" name="room_id" data-placeholder="<?php _e( 'Enter room name', 'opal-hotel-room-booking' ); ?>">
								<# if ( data.room_id ) { #>
							...
							<!-- <select name="qty" class="qty"> -->
								<# if ( data.quantity ) { #>
									<select name="qty" class="qty">
										<option value="0"><?php _e( 'Quantity', 'opal-hotel-room-booking' ); ?></option>
							...
							<!-- <select name="adult"> -->
								<# if ( data.adults ) { #>
									<select name="adult">
										<option value="0"><?php _e( 'Adults', 'opal-hotel-room-booking' ); ?></option>
							...
							<!-- <select name="child"> -->
								<# if ( data.childs ) { #>
									<select name="child">
										<option value="0"><?php _e( 'Childs', 'opal-hotel-room-booking' ); ?></option>
					...
					<div id="container">
						<div class="section opalhotel_datepick_wrap">
							<input type="text" name="arrival" class="opalhotel-has-datepicker" value="{{ data.arrival }}" placeholder="<?php _e( 'Arrival', 'opal-hotel-room-booking' ); ?>" />
							<input type="text" name="departure" class="opalhotel-has-datepicker" value="{{ data.departure }}" placeholder="<?php _e( 'Departure', 'opal-hotel-room-booking' ); ?>" />
							<button class="order_check_avaibility button"><?php _e( 'Check Avaibility', 'opal-hotel-room-booking' ); ?></button>
						</div>
						<div class="section packages">
							<# if ( data.packages ) { #>
								<h5><?php _e( 'Packages', 'opal-hotel-room-booking' ); ?></h5>
							...
							<# } #>
							<# if ( data.hotels ) { #>
								<h5><?php _e( 'Hotels', 'opal-hotel-room-booking' ); ?></h5>
								<select name="hotel">
									<# for( var i = 0; i < Object.keys( data.hotels ).length; i++ ) { #>
					...
					<footer class="center">
						...
						<input type="hidden" name="order_id" value="{{ data.order_id }}" />
						<input type="hidden" name="order_item_id" value="{{ data.order_item_id }}" />
						<button type="submit" class="opalhotel-button opalhotel-button-submit button button-primary" disabled><?php _e( 'Yes', 'opal-hotel-room-booking' ); ?></button>
						<button type="reset" class="opalhotel-button-cancel opalhotel_button_close button"><?php _e( 'No', 'opal-hotel-room-booking' ) ?></button>
