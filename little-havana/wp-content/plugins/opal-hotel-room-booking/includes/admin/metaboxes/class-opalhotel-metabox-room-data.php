...
class OpalHotel_MetaBox_Room_Data {
			...
			<?php foreach ( $tab_panels as $key => $tab ) : ?>
				...
				<?php if ( $key === 'general' ) : ?>
					...
					<div id="general_room_data" class="opalhotel_room_data_panel active">
						...
						<?php if ( opalhotel_enable_hotel_mode() ) : ?>
							<div class="opalhotel_field_group">
								<?php
									...
									$selected = get_post_meta( $post->ID, '_hotel' );
									if ( ! $selected ) $selected = array();
									if ( $hotels ) : ?>
										<label for="hotels"><?php _e( 'Hotels', 'opal-hotel-room-booking' ); ?></label>

										<select name="_hotels[]" multiple="multiple" id="hotels">
											<?php foreach ( $hotels as $hotel ) : setup_postdata( $hotel ); ?>

												<option value="<?php echo esc_attr( $hotel->ID ) ?>"<?php echo ( in_array( $hotel->ID, $selected ) ? ' selected' : '' ) ?>><?php echo esc_html( $hotel->post_title ); ?></option>
						...
						<!-- quantity -->
						<div class="opalhotel_field_group">
							<label for="number_of_rooms"><?php _e( 'Number of room', 'opal-hotel-room-booking' ); ?></label>
						...
						<!-- adults -->
						<div class="opalhotel_field_group">
							<label for="adults"><?php _e( 'Capacities', 'opal-hotel-room-booking' ); ?></label>
						...
						<!-- childrens -->
						<div class="opalhotel_field_group">
							<label for="childrens"><?php _e( 'Childrens', 'opal-hotel-room-booking' ); ?></label>
						...
						<!-- Minimum night -->
						<div class="opalhotel_field_group">
							<label for="min_night"><?php _e( 'Minimum night', 'opal-hotel-room-booking' ); ?></label>
						...
						<!-- Bed -->
						<div class="opalhotel_field_group">
							<label for="bed"><?php _e( 'Bed', 'opal-hotel-room-booking' ); ?></label>
					...
					<div id="extra_amenities" class="extra_amenities opalhotel_room_data_panel">
						<table style="width:100%">
							...
							<?php foreach( $labels as $key => $value ) : ?>
								<tr class="extra_amenities_ipts">
									<td>
										<div class="opalhotel_field_group">
											<label  ><?php _e( 'Label', 'opal-hotel-room-booking' ); ?></label>
											<input name="_extra_amenities_label[]"  class="opalhotel_input_field" type="text" value="<?php echo $value; ?>"  />
										</div>
									</td>
									<td>
										<div class="opalhotel_field_group">
											<label ><?php _e( 'Content', 'opal-hotel-room-booking' ); ?></label>
											<input name="_extra_amenities_content[]"  class="opalhotel_input_field" type="text" value="<?php echo isset($contents[$key])?($contents[$key]):"";?>"  />
										</div>
									</td>
									<td>
										<div class="opalhotel_field_group">
											<div class="btn-remove button" data-confirmed="<?php _e( 'Are you sure to delete?','opal-hotel-room-booking' ); ?>"><?php _e( 'Remove', 'opal-hotel-room-booking' ); ?></div>
										</div>
									</td>
								</tr>
							<?php endforeach; ?>
						...
						<div class="extra_amenities_action">
							<div class="btn-action button button-primary button-large"><?php _e('Add'); ?></div>
						</div>
						<br>
					</div>
					...
					<div class="opalhotel_room_data_panel" id="amenities">
							<!-- View -->
						<div class="opalhotel_field_group">
							<label for="room_size"><?php _e( 'Room Size', 'opal-hotel-room-booking' ); ?></label>
						...
						<div class="opalhotel_field_group">
							<label for="view"><?php _e( 'View', 'opal-hotel-room-booking' ); ?></label>
				...
				<?php elseif ( $key === 'discount' ) : ?>
					...
					<div id="discount_room_data" class="opalhotel_room_data_panel">
						<table class="opalhotel_metabox_table" id="opalhotel_discount_price">
							<thead>
								<tr>
									<th colspan="5"><?php _e( 'Set Discount Price', 'opal-hotel-room-booking' ); ?></th>
								</tr>
								<tr>
									<th><?php _e( 'Type', 'opal-hotel-room-booking' ); ?></th>
									<th><?php _e( 'Unit', 'opal-hotel-room-booking' ); ?></th>
									<th><?php _e( 'Sale', 'opal-hotel-room-booking' ); ?></th>
									<th><?php _e( 'Discount', 'opal-hotel-room-booking' ); ?></th>
									<th>&nbsp;</th>
								</tr>
							</thead>
								...
								<?php if ( $discounts = $opalhotel_room->discounts ) : ?>
									...
									<?php foreach ( $discounts as $k => $discount ) : ?>
										<tr>
											...
											<td>
												<a href="#" class="button opalhotel_remove_extra"><?php _e( 'Remove', 'opal-hotel-room-booking' ); ?></a>
											</td>
										</tr>
									<?php endforeach; ?>
							...
							<tfoot>
								<tr>
									<th colspan="5">
										<a href="#" class="button opalhotel_add_extra" id="opalhotel_add_discount" data-template="opalhotel-discount"><?php _e( 'Add new', 'opal-hotel-room-booking' ); ?></a>
									</th>
								</tr>
							</tfoot>
						</table>
				...
				<?php elseif ( $key === 'extra_price' ) : ?>
					...
					<div id="extra_price_room_data" class="opalhotel_room_data_panel">
						...
						<table class="opalhotel_metabox_table" id="opalhotel_extra_adult_price">
							<thead>
								<tr>
									<th colspan="3"><?php _e( 'Adults', 'opal-hotel-room-booking' ); ?></th>
								</tr>
								<tr>
									<th><?php _e( 'Qty', 'opal-hotel-room-booking' ); ?></th>
								...
								<tr>
									<th colspan="3">
										<a href="#" class="button opalhotel_add_extra" id="opalhotel_add_extra_adult_price" data-template="opalhotel-extra-price" data-name="_extra_adults">
											<?php _e( 'Add new', 'opal-hotel-room-booking' ); ?>
										</a>
									</th>
								</tr>
							</tfoot>
						...
						<table class="opalhotel_metabox_table" id="opalhotel_extra_child_price">
							<thead>
								<tr>
									<th colspan="3"><?php _e( 'Childrens', 'opal-hotel-room-booking' ); ?></th>
								</tr>
								<tr>
									<th><?php _e( 'Qty', 'opal-hotel-room-booking' ); ?></th>
									<th><?php printf( __( 'Price (%s)', 'opal-hotel-room-booking' ), opalhotel_get_currency_symbol() ); ?></th>
				...
				<?php elseif ( $key === 'package' ) : ?>
					...
					<div id="package_room_data" class="opalhotel_room_data_panel">
						...
						<!-- quantity -->
						<div class="opalhotel_field_group">
							<label for="opalhotel_room_package"><?php _e( 'Package', 'opal-hotel-room-booking' ); ?></label>
							...
							<a href="#" class="button" id="opalhotel_room_add_package"><?php _e( 'Add Package', 'opal-hotel-room-booking' ); ?></a>
						</div>
								...
								<?php foreach ( $packages as $k => $package ) : ?>
									...
									<div class="opalhotel_sortable">
										...
										<div class="package_action">
											<a href="#" class="button remove_package"><?php _e( 'Remove', 'opal-hotel-room-booking' ) ?></a>
										</div>
	...
	public static function save( $post_id, $post ) {
		...
		if ( $post->post_type !== 'opalhotel_room' || empty( $_POST ) ) {
			return;
		}
		/* delete post meta */
		delete_post_meta( $post_id, '_hotels' );
		if ( ! empty( $_POST['_hotels'] ) ) {
			foreach ( $_POST['_hotels'] as $hotel_id ) {
				add_post_meta( $post_id, '_hotel', $hotel_id );
			}
		}
		/* set quantity */
