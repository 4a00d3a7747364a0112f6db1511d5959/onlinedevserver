	...
	function opalhotel_booking_manage_posts_custom_columns( $column ) {
		...
		switch ( $column ) {
			...
			case 'reservation_date':
				...
				break;
			case 'total':
				echo opalhotel_format_price( $order->get_total(), $order->payment_currency );
				echo sprintf( __( '<br><small>Via %s</small>' ), $order->payment_method_title );
				break;
			case 'status':
...
if ( ! function_exists( 'opalhotel_order_restrict_manage_posts' ) ) {
	function opalhotel_order_restrict_manage_posts() {
			if ( ! isset( $_GET['post_type'] ) || $_GET['post_type'] !== 'opalhotel_booking' ) {
				return;
			}
			$email = isset( $_GET['_customer_email'] ) ? sanitize_text_field( $_GET['_customer_email'] ) : '';
			$arrival = isset( $_GET['arrival_date'] ) ? sanitize_text_field( $_GET['arrival_date'] ) : '';
			$arrival_timestamp = isset( $_GET['arrival_date_timestamp'] ) ? sanitize_text_field( $_GET['arrival_date_timestamp'] ) : '';
			$departure = isset( $_GET['departure_date'] ) ? sanitize_text_field( $_GET['departure_date'] ) : '';
			$departure_timestamp = isset( $_GET['departure_date_timestamp'] ) ? sanitize_text_field( $_GET['departure_date_timestamp'] ) : '';
		...
		?>
			...
			<div class="opalhotel_datepick_wrap">
				<input name="arrival_date" value="<?php echo esc_attr( $arrival ); ?>" data-min-date="false" class="opalhotel_arrival_date opalhotel-has-datepicker" data-end="opalhotel-departure-date" type="text" placeholder="<?php esc_attr_e( 'Arrival Date', 'opal-hotel-room-booking' ); ?>" />
				<input name="arrival_date_timestamp" type="hidden" value="<?php echo esc_attr( $arrival_timestamp );?>" />
				<input name="departure_date" value="<?php echo esc_attr( $departure ); ?>" data-min-date="false" class="opalhotel-departure-date opalhotel-has-datepicker" data-start="opalhotel_arrival_date" type="text" placeholder="<?php esc_attr_e( 'Departure Date', 'opal-hotel-room-booking' ); ?>" />
				<input name="departure_date_timestamp" type="hidden" value="<?php echo esc_attr( $departure_timestamp );?>" />
			</div>
		<?php
	}
}
...
if ( ! function_exists( 'opalhotel_order_posts_join_paged' ) ) {
	...
	function opalhotel_order_posts_join_paged( $join ) {
		if ( ! is_admin() || ! isset( $_GET['post_type'] ) || $_GET['post_type'] !== 'opalhotel_booking' ) {
			return $join;
		}
		global $wpdb;
		...
		/* search arrival date */
		if ( isset( $_GET['arrival_date'], $_GET['arrival_date_timestamp'] ) && $_GET['arrival_date_timestamp'] ) {
			$arrival_date = date( 'Y-m-d', sanitize_text_field( $_GET['arrival_date_timestamp'] ) );
	        $join .= "
		...
		if ( isset( $_GET['departure_date'], $_GET['departure_date_timestamp'] ) && $_GET['departure_date_timestamp'] ) {
			$departure_date = date( 'Y-m-d', sanitize_text_field( $_GET['departure_date_timestamp'] ) );
	        $join .= "
