...
add_action( 'init', 'opalhotel_set_table_name', 0 );
add_action( 'switch_blog', 'opalhotel_set_table_name', 0 );
if ( ! function_exists( 'opalhotel_set_table_name' ) ) {
	function opalhotel_set_table_name() {
		global $wpdb;
...
function opalhotel_format_price( $price, $currency = true ) {
	...
	$price_thousands_separator = get_option( 'opalhotel_price_thousands_separator', 'left' );
	$price_decimals_separator  = get_option( 'opalhotel_price_decimals_separator', 'left' );
	$price_number_of_decimal   = get_option( 'opalhotel_price_number_of_decimal', 'left' );
	if ( ! is_numeric( $price ) )
		$price = 0;
...
if ( ! function_exists( 'opalhotel_is_room_taxonomy' ) ) {
	...
	function opalhotel_is_room_taxonomy() {
		return is_tax( get_object_taxonomies( 'opalhotel_room' ) );
	}
}
...
if ( ! function_exists( 'opalhotel_is_hotel_taxonomy' ) ) {
	...
	function opalhotel_is_hotel_taxonomy() {
		return is_tax( get_object_taxonomies( 'opalhotel_hotel' ) );
	}
}
	...
	function opalhotel_get_max_adults() {
		...
		if ( ! $max ) {
			...
			$sql = $wpdb->prepare( "
				...
				", '_adults', 'opalhotel_room', 'publish' );
		...
		if ( ! $max ) {
			...
			$sql = $wpdb->prepare( "
					...
					WHERE
						post.post_type = %s
						AND post.post_status = %s
				", '_childrens', 'opalhotel_room', 'publish' );
			$max = $wpdb->get_var( $sql );
		}
...
if ( ! function_exists( 'opalhotel_get_averger_rating' ) ) {
	function opalhotel_get_averger_rating( $room_id = null ) {
		...
		$sql = $wpdb->prepare("
			...
			", $room_id, 'opalhotel_rating' );
		$total = $wpdb->get_var( $sql );

		return apply_filters( 'opalhotel_averger_rating_room', $total / $count, $room_id );
	}
}
...
if ( ! function_exists( 'opalhotel_get_hotel' ) ) {
	function opalhotel_get_hotel( $hotel_id = null ) {
		if ( ! $hotel_id || get_post_type( $hotel_id ) !== 'opalhotel_hotel' || get_post_status( $hotel_id ) !== 'publish' ) {
			return;
		}
