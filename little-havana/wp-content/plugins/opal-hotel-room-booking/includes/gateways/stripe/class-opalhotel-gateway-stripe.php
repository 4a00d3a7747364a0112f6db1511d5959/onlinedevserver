 ...
 * @Last Modified time: 2016-05-15 00:21:56
 */
...
class OpalHotel_Gateway_Stripe extends OpalHotel_Abstract_Gateway {
			...
			<div id="opalhotel-stripe-form">
			    <div class="opalhotel-form-group">
			        <label for="cc-number" class="label-field">
			        	<?php _e( 'Card Number', 'opal-hotel-room-booking' ) ?>
			    ...
			    <div class="opalhotel-form-group">
			        <label for="cc-exp" class="label-field">
			        	<?php _e( 'Expiry (MM/YY)', 'opal-hotel-room-booking' ) ?>
			        	<abbr class="required" title="required">*</abbr>
			        </label>
			    ...
			    <div class="opalhotel-form-group">
			        <label for="cc-cvc" class="label-field">
			        	<?php _e( 'Card Code (CVC)', 'opal-hotel-room-booking' ) ?>
			        	<abbr class="required" title="required">*</abbr>
			        </label>
	...
	public function validate_fields( $posted ) {
		...
		if ( ! $this->secret_key || ! $this->publish_key ) {
			$validated[] = '<strong>ERROR</strong>: ' . __( 'Invalid Secret, Publish Stripe key.', 'opal-hotel-room-booking' );
        }
	...
	public function payment_process( $order_id = null, $page = null ) {
        ...
        if ( is_wp_error( $tokens ) || ! $tokens->id ) {
        	return array(
					'status'	=> false,
					'message'	=> '<strong>ERROR</strong>: ' . __( 'Stripe counld not create tooken for your Credit Card Information.', 'opal-hotel-room-booking' )
				);
        }
        ...
        if ( ! $customer_id ) {
            ...
            if ( is_wp_error( $response ) ) {
            	return array(
					'status'	=> false,
					'message'	=> '<strong>ERROR</strong>: ' . $response->get_error_message()
				);
            }
        ...
        if ( $response && ! is_wp_error( $response ) && $response->id ) {
        	...
        	$result['status']	= false;
        	$result['message']	= '<strong>ERROR</strong>: ' . $response->get_error_message();
        }
