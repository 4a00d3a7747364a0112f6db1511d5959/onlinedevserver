...
if ( ! class_exists( 'YITH_WCWL_Popular_Table' ) ) {
    ...
    class YITH_WCWL_Popular_Table extends WP_List_Table {
        ...
        public function prepare_items() {
            ...
            $total_items = $wpdb->get_var( "SELECT COUNT( DISTINCT prod_id ) FROM {$wpdb->yith_wcwl_items}" );
	        ...
	        $query = "SELECT
		             ...
		             LEFT JOIN {$wpdb->posts} AS p ON p.ID = i.prod_id
	        ...
	        if( $search_string ){
		        $query .= " WHERE p.post_title LIKE %s";
