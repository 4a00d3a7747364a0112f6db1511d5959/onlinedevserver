...
class WooGool_Admin_Feed {
    ...
    function xml_get_products( $pagenum = 1, $is_all_products, $products_cat ) {
        $products = array();
        $pagenum  = absint( $pagenum ) ? $pagenum : 1;
        $offset   = ( $pagenum - 1 ) * $this->number_per_page;
        
        if ( $is_all_products == 'all' ) {   
            $products = woogool_get_products( $this->number_per_page, $offset );

        } else if ( $is_all_products == 'category' ) {
    ...
    function get_xml_content( $feed_id ) {
        ...
        $is_all_products  = empty( $feed_settings['_all_products'] ) ? 'all' : reset( $feed_settings['_all_products'] );
        ...
        $products = $this->xml_get_products( $pagenum, $is_all_products, $products_cat );
    ...
    function generate_xml_content( $product, $feed_settings ) {
        ...
        echo "       <item>\n";
        echo "           <g:id><![CDATA[$wc_product->id]]></g:id>\n";
    ...
    function xml_for_product_variation( $wc_product,  $attr, $get_attrs ) {
        ...
        echo  $size_type ? "          <g:size_type><![CDATA[$size_type]]></g:size_type>\n" : '';
        echo  $size_system ? "            <g:size_system><![CDATA[$size_system]]></g:size_system>\n" : '';
        echo  $price ? "         <g:sale_price><![CDATA[$price]]></g:sale_price>\n" : '';
        echo  $is_bundle ? "       <g:is_bundle><![CDATA[$is_bundle]]></g:is_bundle>\n" : '';
        echo  $multipack ? "       <g:multipack><![CDATA[$multipack]]></g:multipack>\n" : '';
        echo  $material ? "       <g:material><![CDATA[$material]]></g:material>\n" : '';
        echo  $pattern ? "       <g:pattern><![CDATA[$pattern]]></g:pattern>\n" : '';
    ...
    function get_product_sale_price( $wc_product ) {
        ...
        $price =  abs( $wc_product->sale_price );
    ...
    function get_product_regular_price( $wc_product ) {
        ...
        $price =  abs( $wc_product->regular_price ); 
        $price = empty( $price ) ? abs( $wc_product->sale_price ) : $price;
    ...
    function get_description( $wc_product ) {
        ...
        if ( $description == 'short_description' ) {
            $description  = $wc_product->post->post_excerpt;
        } else {
    ...
    function get_condition() {
        ...
        return empty( $feed_settings['_condition'] ) ? false : reset( $feed_settings['_condition'] );
    }
