...
$recipient_dsname=$cart_item['recipient_dsname'][0];
		...
		if ( ! empty( $recipient_email ) ) {
			...
			if ( empty( $recipient_user_id ) ) {
				...
				wp_update_user( array( 'ID' => $recipient_user_id, 'first_name' => $recipient_dsname ) );
				
				$reuanme = explode(' ',$recipient_dsname);
				
				if(count($reuanme)>1){
				update_user_meta( $recipient_user_id, 'shipping_first_name', $reuanme[0]);
				update_user_meta( $recipient_user_id, 'shipping_last_name', $reuanme[1]);
		}else{
						update_user_meta( $recipient_user_id, 'shipping_first_name', $recipient_dsname);
						update_user_meta( $recipient_user_id, 'shipping_last_name', '');
					}
