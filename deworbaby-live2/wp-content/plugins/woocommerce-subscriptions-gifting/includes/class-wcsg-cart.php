...
class WCSG_Cart {
			...
			if ( ! empty( $_POST['_wcsgnonce'] ) && wp_verify_nonce( $_POST['_wcsgnonce'], 'wcsg_add_recipient' ) ) {
				...
				foreach ( WC()->cart->cart_contents as $key => $item ) {
					if ( isset( $_POST['recipient_email'][ $key ] ) ) {
						...
						WCS_Gifting::update_cart_item_key( $item, $key, $_POST['recipient_dsname'][ $key ] );
						WCS_Gifting::update_cart_item_key( $item, $key, $_POST['recipnote'][ $key ] );
					}
				}
			} else {
	...
	public static function maybe_display_gifting_information( $cart_item, $cart_item_key ) {
		...
		if ( self::is_giftable_item( $cart_item ) ) {
			...
			$dsname = ( empty( $cart_item['recipient_dsname'] ) ) ? '' : $cart_item['recipient_dsname'];
			$recipnote = ( empty( $cart_item['recipient_recipnote'] ) ) ? '' : $cart_item['recipient_recipnote'];
			wc_get_template( 'html-add-recipient.php', array( 'email_field_args' => WCS_Gifting::get_recipient_email_field_args( $email ), 'id' => $cart_item_key, 'email' => $email, 'dsname' => $dsname[0], 'recipnote' => $recipnote[0] ),'' , plugin_dir_path( WCS_Gifting::$plugin_file ) . 'templates/' );
