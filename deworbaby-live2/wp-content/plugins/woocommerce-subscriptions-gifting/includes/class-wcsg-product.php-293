...
class WCSG_Product {
		...
		if ( isset( $_POST['recipient_email'] ) && ! empty( $_POST['recipient_email'][0] ) ) {
			if ( ! empty( $_POST['_wcsgnonce'] ) && wp_verify_nonce( $_POST['_wcsgnonce'], 'wcsg_add_recipient' ) ) {
				...
				if ( WCS_Gifting::validate_recipient_emails( $_POST['recipient_email'] ) ) {
					...
					$cart_item_data['recipient_dsname'] = $_POST['recipient_dsname'][0];
					$cart_item_data['recipnote'] = $_POST['recipnote'][0];
	...
	public static function add_gifting_option_product() {
		...
		if ( self::is_giftable( $product ) && ! isset( $_GET['switch-subscription'] ) ) {
			...
			if ( ! empty( $_POST['recipient_dsname'][0] ) && ! empty( $_POST['_wcsgnonce'] ) && wp_verify_nonce( $_POST['_wcsgnonce'], 'wcsg_add_recipient' ) ) {
				$dsname = $_POST['recipient_dsname'][0];
			}
			if ( ! empty( $_POST['recipnote'][0] ) && ! empty( $_POST['_wcsgnonce'] ) && wp_verify_nonce( $_POST['_wcsgnonce'], 'wcsg_add_recipient' ) ) {
				$recipnote = $_POST['recipnote'][0];
			}
			wc_get_template( 'html-add-recipient.php', array( 'email_field_args' => WCS_Gifting::get_recipient_email_field_args( $email ), 'id' => 0,  'email' => $email, 'dsname' => $dsname, 'recipnote' => $recipnote ), '', plugin_dir_path( WCS_Gifting::$plugin_file ) . 'templates/' );
