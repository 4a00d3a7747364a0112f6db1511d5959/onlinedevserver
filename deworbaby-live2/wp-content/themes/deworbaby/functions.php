...
if(!empty($user_meta)){
	...
	function discount_when_quantity_greater_than_10 () {
		$coupon_code = 'dewor'.$user->display_name.'baby';
		$page = get_page_by_title($coupon_code, OBJECT, 'shop_coupon');
		if($page->ID==""){
			global $woocommerce;
			global $total_qty;
			$user = wp_get_current_user(); 
			...
			$coupon_code = 'dewor'.$user->display_name.'baby'; // Code
			$amount = $user_meta[0]; // Amount
			...
			if(in_array($new_coupon_id, $woocommerce->cart->applied_coupons)){
